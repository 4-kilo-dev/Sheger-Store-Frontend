...
import { MOCK_BOOKINGS, STATUS_ORDER } from "@/lib/mock-bookings";
...
const STATUS_ACTIONS: Record<
  string,
  { label: string; icon: any; nextStatus: string; role: string; color?: string }
> = {
  RESERVED: {
...
function BookingDetailPage() {
  const { booking } = Route.useLoaderData() as { booking: B };
    ...
    <AppShell>
          ...
          {action && (
            <button
              ...
              style={{
                ...
                color: action.color ? "#fff" : "var(--accent-foreground)",
              }}
            >
      ...
      {showActionModal && action && (
        <div
          ...
          style={{
            borderColor: action.color || "var(--accent)",
            background: `color-mix(in oklab, ${action.color || "var(--accent)"} 6%, var(--surface))`,
          }}
        >
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px]" style={{ color: "var(--text-2)" }}>
                  This will advance the booking from <strong>{booking.status}</strong> to{" "}
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <label className="text-[11px] font-semibold" style={{ color: "var(--text-2)" }}>
                      ...
                      <select
                        ...
                        style={{ borderColor: "var(--border)" }}
                      >
                        <option>Bereket Alemu</option>
                        <option>Robel Hailu</option>
                      </select>
                    </label>
                    <label className="text-[11px] font-semibold" style={{ color: "var(--text-2)" }}>
                      Assign Technician
                      <select
                        ...
                        style={{ borderColor: "var(--border)" }}
                      >
                        <option>Yeabtsega</option>
                        <option>Dawit Mekonnen</option>
                        <option>Yonas Kebede</option>
                        <option>Mahlet Girma</option>
                      </select>
                    </label>
                  </div>
                )}
                ...
                {booking.status === "PREPARATION" && (
                  <div className="mt-3 grid grid-cols-3 gap-3">
                    <label className="text-[11px] font-semibold" style={{ color: "var(--text-2)" }}>
                      Team Leader
                      <input
                        defaultValue={booking.teamLeader}
                        className="mt-1 h-9 w-full rounded-md border bg-[var(--surface-2)] px-2 text-[12px]"
                        style={{ borderColor: "var(--border)" }}
                      />
                    </label>
                    ...
                    <label className="text-[11px] font-semibold" style={{ color: "var(--text-2)" }}>
                      ...
                      <input
                        type="number"
                        defaultValue={booking.mealBudget}
                        className="mt-1 h-9 w-full rounded-md border bg-[var(--surface-2)] px-2 font-mono text-[12px]"
                        style={{ borderColor: "var(--border)" }}
                      />
                    </label>
          ...
          >
            <button
              className="rounded-md px-4 py-2 text-[12px] font-bold transition hover:brightness-110"
              style={{
                background: action.color || "var(--accent)",
                color: action.color ? "#fff" : "var(--accent-foreground)",
              }}
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-4 space-y-4">
        ...
        <Section title="Financial" icon={DollarSign}>
          ...
          <KV
            k="Balance"
            v={b.payment === "PAID" ? "ETB 0" : `ETB ${(b.amount / 2).toLocaleString()}`}
            mono
          />
        ...
        <Section title="Quick Stats" icon={CheckCircle2}>
          <KV
            k="Days to Event"
            v={Math.max(0, Math.ceil((new Date(b.eventDate).getTime() - Date.now()) / 86400000))}
            mono
          />
        ...
        {events.map((e, i) => (
          <div key={i} className="relative flex items-start gap-4">
            <div
              ...
              style={{ color: e.accent ? "var(--accent)" : "var(--text-2)" }}
            >
              {e.t}
            </div>
            <div
      ...
      <div className="col-span-4">
        <Section title="Summary" icon={DollarSign}>
          ...
          <div className="mt-2 border-t pt-2" style={{ borderColor: "var(--border)" }}>
            <KV
              k="Balance Due"
              v={`ETB ${(b.amount - tx.reduce((s, t) => s + t.a, 0)).toLocaleString()}`}
              mono
            />
          </div>
        </Section>
        ...
        {log.map((l, i) => (
          <div key={i} className="flex items-start gap-3 text-[12px]">
            <div
              ...
              style={{ background: l.accent ? "var(--accent)" : "var(--text-3)" }}
            />
            <div className="flex-1">
              ...
              {l.what && (
                <span
                  ...
                  style={{ color: l.accent ? "var(--accent)" : "var(--foreground)" }}
                >
                  {l.what}
                </span>
              )}
