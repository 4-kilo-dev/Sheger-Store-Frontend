...
function InventoryDetail() {
  const { item } = Route.useLoaderData() as { item: (typeof MOCK_INVENTORY)[number] };
  const [tab, setTab] = useState<"Units" | "Movement" | "Maintenance">("Units");
  const units = Array.from({ length: Math.min(item.total, 12) }, (_, i) => ({
    serial: `${item.id}-${String(i + 1).padStart(3, "0")}`,
    state:
      i < item.damaged
        ? "DAMAGED"
        : i < item.damaged + item.onsite
          ? "ONSITE"
          : i < item.damaged + item.onsite + item.reserved
            ? "RESERVED"
            : "AVAILABLE",
    location: i < item.onsite ? "SB047 · Sheraton" : item.location,
  }));
      ...
      <div className="mt-4 grid grid-cols-12 gap-4">
        <div className="col-span-9 rounded-lg border border-border bg-surface">
          ...
          {tab === "Units" && (
            <table className="w-full text-[12px]">
              <thead>
                <tr>
                  {["Serial", "State", "Current location", "Last inspection"].map((h) => (
                    <th
                      key={h}
                      className="border-b border-border px-4 py-3 text-left label-eyebrow"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {units.map((unit, i) => (
                  <tr key={unit.serial} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 font-mono text-text-2">
                      2026-05-{String(28 - i).padStart(2, "0")}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="space-y-4 p-5">
              {[
                ...
                <div key={event} className="flex gap-3">
                  ...
                  <div>
                    <p className="text-[12px] font-semibold">{event}</p>
                    <p className="font-mono text-[10px] text-text-3">
                      2026-06-{String(8 - i).padStart(2, "0")} · Nathan B.
                    </p>
                  </div>
                </div>
              ))}
          ...
          {tab === "Maintenance" && (
            <div className="p-5">
              <div className="flex items-center gap-3 rounded-md border border-border bg-surface-2 p-4">
                ...
                <div>
                  <p className="text-[13px] font-semibold">Next preventive service</p>
        ...
        <aside className="col-span-3 space-y-4">
          <div className="rounded-lg border border-border bg-surface p-4">
            ...
            <p className="mt-1 text-[11px] text-text-2">Main warehouse · Bole</p>
          </div>
          <div className="rounded-lg border border-border bg-surface p-4">
            <div className="mb-3 flex items-center gap-2">
            ...
            <div className="space-y-2 text-[11px]">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{item.lastService}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-2">Next due</span>
                <span className="font-mono">{item.nextService}</span>
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-surface p-4">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="label-eyebrow">Custodian</span>
            </div>
            <p className="text-[13px] font-semibold">Mekonnen T.</p>
            <p className="text-[11px] text-text-2">Storekeeper</p>
          </div>
        </aside>
      </div>
    </AppShell>
