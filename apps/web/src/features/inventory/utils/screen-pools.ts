...
/** True for area units used by LED screen modules (sqm / m²). */
...
/** Heuristic when category metadata is missing — pitch LED panels only. */
export function looksLikeScreenPoolName(name?: string | null): boolean {
  const n = String(name || "");
  if (!n.trim()) return false;
  // Exclude obvious non-screen gear that may mention "panel" loosely
...
/**
 * Keep only LED screen module pools for Screen Type pickers.
 * Never falls back to the full inventory list.
 */
