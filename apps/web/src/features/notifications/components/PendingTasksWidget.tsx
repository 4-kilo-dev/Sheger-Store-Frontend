...
export function PendingTasksWidget() {
    ...
    switch (priority) {
      case "urgent":
        return {
          ...
          bg: "rgba(229, 70, 102, 0.15)",
          border: "1px solid rgba(229, 70, 102, 0.3)",
        };
      case "high":
        return {
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <div className="py-6 text-center text-[11px]" style={{ color: "var(--text-3)" }}>
          Loading tasks...
        </div>
      ) : pendingTasks.length === 0 ? (
        ...
        <div className="space-y-2.5 flex-1 overflow-y-auto max-h-[300px] pr-1 scrollbar-thin">
          {pendingTasks.map((t) => {
            ...
            if (
              t.eventType === "booking.technical_allocated" ||
              display.linkTo?.startsWith("/bookings/")
            ) {
              actionLabel =
                t.eventType === "booking.technical_allocated" ? "Open to quote" : "Review Booking";
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p
                    className="leading-relaxed leading-5 text-[11px]"
